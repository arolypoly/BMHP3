...
class DxlChain:
    ...
    def _read(self, id, address, size):
        ...
        if len(data) != size:
            raise DxlCommunicationException(
                'Read command did not obtain the %d bytes expected: got %d bytes' % (size, len(data)))
        return data

    def _write(self, id, address, values):
        """Write data to a motor registers"""
        self._comm(id, [Dxl.CMD_WRITE_DATA, address, values])
    ...
    def sync_write_pos_speed(self, ids, positions, speeds):
        ...
        for id in ids:
            ...
            if regpos == None:
                ...
                if regpos.address != r.address:
                    raise DxlConfigurationException(
                        "Synchronized write %s impossible on chain, mismatch in register address for motor ID %d" % (
                            reg_name, id))
                if regpos.size != r.size:
                    raise DxlConfigurationException(
                        "Synchronized write %s impossible on chain, mismatch in register size for motor ID %d"(reg_name,
                                                                                                               id))

            reg_name = "moving_speed"
            if reg_name not in m.registers.keys():
                raise "Synchronized write %s impossible on chain, register absent from motor ID %d" % (reg_name, id)(
        ...
        if (regpos.address + regpos.size) != regspeed.address:
            raise DxlConfigurationException(
                "Synchronized write goal_pos/moving_speed impossible on chain, registers are not consecutive")
    ...
    def set_configuration(self, conf):
        ...
        for id in conf.keys():
            sid = id
            iid = int(sid)
            if iid not in self.motors.keys(): raise DxlConfigurationException(
                "Cannot find motor ID %d to be configured" % iid)
            ...
            for (name, val) in conf[sid].items():
                ...
                if not 'w' in reg.mode:  # read only: generate error if setting is EEPROM
                    if reg.eeprom:
                        raise DxlConfigurationException(
                            "Invalid EEPROM value in motor ID %d register %s: current=%d expected=%d" % (
                                iid, name, current, val))
                    else:
                        pass

            # Check/Set all registers
            for (name, val) in conf[sid].items():
                if name not in motor.registers.keys(): raise DxlConfigurationException(
                    "Cannot configure missing register %s on motor ID %d" % (name, iid))
                reg = motor.registers[name]
                current = self.get_reg(iid, name)
                if current == val: continue
                # Value has to be changed
                if not 'w' in reg.mode:  # read only: generate error if setting is EEPROM
                    ...
                    if reg.eeprom:
                        logging.info(
                            "Changed (EEPROM register %s from %d to %d on motor ID %d" % (name, current, val, iid))
