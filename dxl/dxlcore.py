...
class ModelRegisteringMetaclass(type):
    def __new__(mcs, name, bases, attrs):
        inst = type.__new__(mcs, name, bases, attrs)
...
class DxlElement(object):
    ...
    def registerModel(cls, model_number, model_cls):
        if model_number not in cls.DxlModels.keys():
            ...
            logging.info(
                "Registered Dynamixel Element model %s (%d): " % (model_cls.model_name, model_number) + str(model_cls))

    @classmethod
    def instantiateMotor(cls, id, model_number):
        if model_number not in cls.DxlModels.keys():
            raise DxlConfigurationException(
                "Cannot instantiate non registered element model %d on ID %d" % (model_number, id))
    ...
    def setRegisterCmd(self, name, value):
        ...
        if r.size != len(value):
            raise DxlConfigurationException(
                "Model %s register %s has size %d: passed size %d" % (self.model_name, name, r.size, len(value)))
