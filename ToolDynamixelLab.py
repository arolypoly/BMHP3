...
class MainWindow:
    ...
    def buildMenu(self, root):
        ...
        settingsmenu.add_checkbutton(label="Use Ping Broadcast", onvalue=True, offvalue=False,
                                     variable=self.doBroadcast)
    ...
    def factoryReset(self):
        ...
        if id < 0:
            ...
            if 1 in self.chain.motors.keys():
                answer = tkMessageBox.askyesno("ID Conflict",
                                               "Warning: the motor ID 1 obtained after factory reset is already attributed on your chain, are you sure you want to proceed?")
                if not answer:
                    do = False
            if do:
    ...
    def close(self):
        ...
        if self.chain:
            try:
                ...
                logging.warning("WARNING: could not close chain")
    ...
    def scan(self):
        ...
        for rate in rates:
            ...
            try:
                ...
                tkMessageBox.showerror("Communication Error",
                                       "Could not communicate with motor (could be due to overlapping IDs, try on single motors): \n" + str(
                                           e))
                return
            finally:
                self.close()
